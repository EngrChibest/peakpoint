...
const Header = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${showSolidHeader ? 'bg-white/95 backdrop-blur-md shadow-md py-3' : 'bg-transparent py-5'}`}>
      <div className="container flex-between">
        <Link to="/" className="flex items-center gap-2">
          <div className="logo-container">
            <img src={logo} alt="PPIS Logo" />
          </div>
          <div className={`flex flex-col font-baloo transition-colors duration-300 ${showSolidHeader ? 'text-primary' : 'text-secondary'}`}>
        ...
        <nav className="hidden md:flex items-center gap-8">
          <Link to="/" className={`font-semibold transition-colors hover:underline underline-offset-4 hover:opacity-80 ${showSolidHeader ? 'text-primary' : 'text-secondary'}`}>Home</Link>
          <Link to="/about" className={`font-semibold transition-colors hover:underline underline-offset-4 hover:opacity-80 ${showSolidHeader ? 'text-primary' : 'text-secondary'}`}>About</Link>
          <Link to="/admissions" className={`font-semibold transition-colors hover:underline underline-offset-4 hover:opacity-80 ${showSolidHeader ? 'text-primary' : 'text-secondary'}`}>Admissions</Link>
          <Link to="/academics" className={`font-semibold transition-colors hover:underline underline-offset-4 hover:opacity-80 flex items-center gap-1 ${showSolidHeader ? 'text-primary' : 'text-secondary'}`}>
            Academics <ChevronDown size={16} />
          </Link>
          <Link to="/facilities" className={`font-semibold transition-colors hover:underline underline-offset-4 hover:opacity-80 ${showSolidHeader ? 'text-primary' : 'text-secondary'}`}>Campus Life</Link>
          <Link to="/news" className={`font-semibold transition-colors hover:underline underline-offset-4 hover:opacity-80 ${showSolidHeader ? 'text-primary' : 'text-secondary'}`}>News</Link>
          <Link to="/contact" className={`font-semibold transition-colors hover:underline underline-offset-4 hover:opacity-80 ${showSolidHeader ? 'text-primary' : 'text-secondary'}`}>Contact Us</Link>
        ...
        {/* Mobile Toggle */}
        <button className={`md:hidden ${showSolidHeader ? 'text-primary' : 'text-secondary'}`} onClick={() => setIsOpen(!isOpen)}>
